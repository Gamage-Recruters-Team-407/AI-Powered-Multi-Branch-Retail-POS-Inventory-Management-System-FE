...
const normalizeProduct = (p, index) => {
  if (p.rank !== undefined && p.product_name !== undefined) return p;

  // Backend format: { _id, productName, totalQuantity, totalRevenue, ... }
  const revenue = p.totalRevenue ?? p.revenue ?? 0;
  const revenueFormatted =
    typeof revenue === 'number'
      ? `Rs. ${revenue.toLocaleString('en-US', { minimumFractionDigits: 0 })}`
      : String(revenue);

  return {
    rank: index + 1,
    product_name: p.productName ?? p.product_name ?? p.name ?? 'Unknown',
    category: p.category ?? p.categoryName ?? '—',
    units_sold: p.totalQuantity ?? p.units_sold ?? p.quantity ?? 0,
    revenue: revenueFormatted,
    times_sold: p.transactionCount ?? p.times_sold ?? 0,
    growth: p.growth ?? 0,
  };
};

const TopProducts = ({ data }) => {
  // data.top_products → backend array | data.sales?.topProducts → nested | fallback demo
  const rawProducts =
    data?.top_products ||
  ...
  return (
    <div className="tp-card">
      ...
      <div className="tp-table">
        ...
        {products.map((p, i) => (
          <div key={i} className="tp-row">
            <div className={`tp-rank rank-${Math.min(p.rank ?? i + 1, 4)}`}>
              {p.rank ?? i + 1}
            </div>
            <div className="tp-product">
              <div className="tp-name">{p.product_name}</div>
            ...
            <div>
              <div className="tp-units">
                {(p.units_sold ?? 0).toLocaleString()}
              </div>
              <div className="tp-times">{p.times_sold ?? 0} orders</div>
            </div>
            <div className="tp-rev">{p.revenue}</div>
            <div className={`tp-growth ${(p.growth ?? 0) >= 0 ? 'up' : 'down'}`}>
              {(p.growth ?? 0) >= 0 ? '↑' : '↓'} {Math.abs(p.growth ?? 0)}%
            </div>
          </div>
        ))}
      </div>
